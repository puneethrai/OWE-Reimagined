(function ($) {
    ...
    Selectpicker.DEFAULTS = {
        ...
        countSelectedText: function (numSelected, numTotal) {
            /*jslint unparam:true*/
            return (parseInt(numSelected, 10) === 1) ? "{0} item selected" : "{0} items selected";
        },
        maxOptionsText: function (numAll, numGroup) {
            var arr = [];

            arr[0] = (parseInt(numAll, 10) === 1) ? 'Limit reached ({n} item max)' : 'Limit reached ({n} items max)';
            arr[1] = (parseInt(numGroup, 10) === 1) ? 'Group limit reached ({n} item max)' : 'Group limit reached ({n} items max)';
